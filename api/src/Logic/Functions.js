...
module.exports={
    apiRecipesname: async(name)=>{//traigo todas las recetas de la api que contengan el name ingresado  
    ...
    dBRecipes: async()=>{//traigo la info de mi base de datos
        return await Recipe.findAll({//obtengo de mi modelo Recipe todas las recetas creadas, donde incluya al modelo Diet, para que se realice la relacion y traiga la dieta al crear un personaje
            include:{
                model: Diet,
                attributes:['name'],
                through:{
    ...
    allRecipesAndName: async (name) => {//uno a todos las recetas (api/db)
            const apiInf= await module.exports.apiRecipesname(name.toLowerCase());
            const dBInf= await module.exports.dBRecipes();
            const dBInfname=await dBInf.filter(e=>e.title.toLowerCase().includes(name.toLowerCase()))
    ...
    idRecipes: async (id)=>{
        ...
        if (aux.length>15) {
            ...
            console.log('esto es lo que envia al final',idDB.Diets.map(e=>e.dataValues.name))
            const diets=await idDB.Diets.map(e=>e.dataValues.name)
            const {title, image, dishTypes, summary, spoonacularScore, healthScore, steps}= idDB            
            return {title, image, diets, dishTypes, summary, spoonacularScore, healthScore, steps}
        } else {
            console.log('esto se mi id: ', id)
            const apiID= await axios.get(`https://api.spoonacular.com/recipes/${id}/information?apiKey=${API_KEYS}`)
            console.log('esto llega del error: ', apiID.data)
            const {title, image, diets, dishTypes, summary, spoonacularScore, healthScore, analyzedInstructions}= await apiID.data
            let steps=analyzedInstructions.length!==0? analyzedInstructions[0].steps.map(e=>e.step):''
            steps!=='' && await steps.join('. ')
            return {title, image, diets, dishTypes, summary, spoonacularScore, healthScore, steps}
        }      
    },
    uploadDietsToDataBase: async()=>{
        let dietstypes=[  "gluten free", "ketogenic", "vegetarian", "lacto vegetarian","ovo vegetarian", "vegan", "pescatarian", "paleolithic", "primal", "fodmap friendly", "whole 30"]
    ...
    // post: async ()=>{
    //     title, summary, spoonacularScore, healthScore, steps, diets
    // }
    // typesDiets: async()=>{

    // },
    //busco todos los perros de la db
